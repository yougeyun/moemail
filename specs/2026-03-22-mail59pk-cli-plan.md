# mail59pk CLI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an agent-first CLI tool that wraps mail59pk's existing REST API, published as an npm package.

**Architecture:** CLI lives in `packages/cli/`, uses commander for arg parsing, calls mail59pk API via `fetch` with `X-API-Key` auth. One server-side fix needed for the send endpoint auth.

**Tech Stack:** TypeScript, Bun (build), commander (CLI framework), Node built-in `fetch` and `fs`

**Spec:** `specs/2026-03-22-mail59pk-cli-design.md`
...
│   ├── config.ts         # Config file read/write (~/.mail59pk/config.json) + env override
...
```json
{
  "name": "mail59pk-cli",
  "version": "0.1.0",
  "description": "Agent-first CLI for mail59pk temporary email service",
  "type": "module",
  "bin": {
    "mail59pk": "dist/index.js"
  },
  "scripts": {
...
program
  .name("mail59pk")
  .description("mail59pk CLI — Agent-friendly temporary email tool")
  .version("0.1.0");
...
```bash
cd packages/cli
bun run src/index.ts config set api-url https://mail.59pk.net
bun run src/index.ts config set api-key mk_test123
bun run src/index.ts config list
cat ~/.mail59pk/config.json
```
...
async function request(
  ...
  if (!config.apiUrl) {
    log("Error: API URL not configured. Run: mail59pk config set api-url <url>");
    process.exit(2);
  }
  if (!config.apiKey) {
    log("Error: API Key not configured. Run: mail59pk config set api-key <key>");
    process.exit(2);
  }
...
bun run src/index.ts create --domain mail.59pk.net --expiry 1h --json
```
...
Include:
- One-line description: "Agent-first CLI for mail59pk temporary email service"
